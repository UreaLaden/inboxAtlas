...
| `classify label-analysis` | Shows mailbox-scoped Gmail label frequency so operators can author manual `pattern-type=label` seeds from observed labels; known Gmail labels use synced display names when available, and `--top-domains N` expands each label into its top N sender domains |
...
- `classify messages` is the per-message review surface; JSON includes `message_id`, and CSV exports `MessageID,Timestamp,Sender,Domain,Subject,Intent,Category,HasAttachment`.
...
Example first-party inference provider workflow:

```bash
go build -o ./bin/ai-inference-provider ./cmd/ai-inference-provider

export OPENAI_API_KEY=sk-...
./bin/inboxatlas classify infer \
  --account <id|alias> \
  --provider-command ./bin/ai-inference-provider
```
...
make test           # go test ./...
make build          # build inboxatlas, ia, openai-summary-provider, and ai-inference-provider
