...
| `classify messages` | Lists per-message classification rows with optional category and intent filters, attachment presence, and table/csv/json output |
...
- `classify messages` is the per-message review surface; use `--category`, `--intent`, and `--format json` or `--format csv` for automation-friendly filtering/export.
