...
| `classify messages` | Lists per-message classification rows with optional category and intent filters, message IDs, attachment presence, and table/csv/json output |
...
- `classify messages` is the per-message review surface; JSON includes `message_id`, and CSV exports `MessageID,Timestamp,Sender,Domain,Intent,Category,HasAttachment` for automation-friendly filtering/export.
